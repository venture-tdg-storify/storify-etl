create or replace procedure arteli_db_ingestion.tdg_ingestion.load_raw_inventory()
...
new_files as (
    ...
    loaded_files as(
        ...
        from arteli_db_ingestion.tdg_ingestion.raw_inventory
    )
    ...
    select
        ...
        t.$20 as qty_on_hand,
        t.$21 as trans_date
    from @tdg_stage/inventory/ (file_format => s3_csv_inventory) t
...
try {
    ...
    var email_command = "call arteli_db_ingestion.ops.enqueue_alert_stage('Task Failure - TDG load_raw_inventory','" + error_message + "')";
