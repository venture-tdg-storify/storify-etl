create or replace procedure arteli_db_ingestion.tdg_ingestion.load_raw_locations()
...
as
$$
var sql_command = `insert into arteli_db_ingestion.tdg_ingestion.raw_locations (
    _file,
    _line,
    _modified,
    location_id,
...
with base_files as(
    ...
    from arteli_db_ingestion.tdg_ingestion.raw_locations
)
...
select
    ...
    t.$5 as city,
    t.$6 as address_1
from  @tdg_stage/locations (file_format => s3_csv) t
...
try {
    ...
    var email_command = "call arteli_db_ingestion.ops.enqueue_alert_stage('Task Failure - TDG load_raw_locations','" + error_message + "')";
